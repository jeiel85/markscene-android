...
> **내 하루의 장면을 검색 가능한 기억으로** — Android 앱

기본 기능은 API Key 없이 로컬 우선으로 동작합니다. 사용자는 사진을 찍거나 선택한 뒤, 앱이 생성한 빠른 태그 초안을 수정하고 저장할 수 있습니다. 장면별 기억 유형(아이디어, 업무, 영수증 등)을 지정하고, 나중에 다시 볼 기록을 Recall Box에 모아둘 수 있습니다. 고급 AI 분석은 사용자가 직접 API Key를 입력하는 BYOK(Bring Your Own Key) 방식의 선택 기능으로 제공합니다.
...
> 사진을 찍으면 자동 태그가 생성되고, 기억 유형을 지정해 나중에 쉽게 찾을 수 있는 **개인 기억 저장소**
...
태그 칩 수정/삭제/추가
  ↓
기억 유형(Memory Type) 선택
  ↓
로컬 기록 저장
  ↓
날짜별 Scene Timeline에서 확인
  ↓
태그, 제목, 메모, 기억 유형으로 검색
  ↓
Recall Box에서 다시 볼 기록 관리
```
...
### 📅 Scene Timeline
- 기록을 날짜별로 그룹화된 타임라인으로 표시
- 오늘/어제/날짜별 섹션 구분

### 🏷 Memory Tags (기억 유형)
- 아이디어, 업무, 가족, 영수증 등 14종 기억 유형
- 기록 저장 시 복수 선택 가능

### 🔖 Recall Box (다시 보기)
- 나중에 다시 볼 기록을 별도 탭에 모아 관리
- 저장 시 Recall 토글로 간편 지정

### 📊 Daily Scene Recap
- 오늘 화면에서 일일 기록 요약 카드 제공
- 많이 나온 태그 TOP 5 표시
