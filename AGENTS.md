...
```text
Project Name: MarkScene
Repository: https://github.com/jeiel85/markscene-android.git
...
Version Files: app/build.gradle.kts, gradle/libs.versions.toml, README.md, CHANGELOG.md
Build/Test Commands: ./gradlew test, ./gradlew lint, ./gradlew assembleDebug
...
### MarkScene 프로젝트 고유 정책 요약

이 저장소는 **사진을 검색 가능한 비주얼 메모로 바꾸는 Android 앱**을 개발합니다. 기본 경험은 로컬 우선으로 동작하며, API Key 없이도 사진 기록, 자동 태그 초안, 태그 수정, 로컬 검색이 가능해야 합니다.

비협상 원칙:

- Android `applicationId`와 루트 패키지는 기본적으로 `com.markscene.app`을 사용합니다.
- 앱은 API Key 없이 기본 기능과 로컬 고급 AI 분석이 동작해야 합니다.
- 고급 AI 분석은 앱에서 다운로드한 로컬 LLM/VLM 모델만 사용합니다.
- AI 분석은 사용자가 명시적으로 실행한 경우에만 수행합니다.
- 기본 사진 가져오기는 Android Photo Picker를 우선합니다.
- `MANAGE_EXTERNAL_STORAGE`를 추가하지 않습니다.
- 광범위한 사진/동영상 권한을 추가하지 않습니다.
- 사용자의 전체 갤러리를 스캔하지 않습니다.
- 로그인, 계정, 클라우드 동기화, 광고, 분석 SDK, crash reporting SDK는 명시 승인 전까지 추가하지 않습니다.
- 개발자 소유 기본 API Key나 외부 AI 분석용 API Key 입력 경로를 앱에 넣지 않습니다.
- 라이선스가 필요한 모델 다운로드용 read token은 기기 내부에만 암호화 저장합니다.
- 모델 다운로드 토큰, 이미지 바이트, 프롬프트, AI 응답, 민감 데이터는 로그에 남기지 않습니다.
- AI와 로컬 태그 결과는 확정 사실이 아니라 수정 가능한 제안으로 표시합니다.
- 사용자 사진, 위치, 메모, 태그의 외부 전송 여부는 앱 안에서 명확히 고지해야 합니다.
...
## 30. MarkScene 전용 구현 규칙

이 섹션은 MarkScene 앱 구현 시 항상 적용합니다.

### 30.1 기본 제품 경험

- 첫 실행에서 외부 AI API Key 입력을 요구하지 않습니다.
- 기본 플로우는 `사진 촬영/선택 → 로컬 태그 초안 → 태그 수정 → 로컬 저장 → 검색`입니다.
- 외부 AI API Key가 없어도 사용자는 앱의 핵심 가치와 로컬 고급 분석을 경험할 수 있어야 합니다.
- 고급 AI 분석은 설정 또는 분석 화면에서 선택적으로 안내합니다.
...
위치 기능은 MVP 범위가 아니며, 추후 추가하더라도 사용자가 특정 기록에 위치를 추가할 때만 요청합니다.

### 30.3 로컬 태그 처리

- 초기 구현은 `MockLocalImageTagger`로 시작할 수 있습니다.
- 실제 온디바이스 태깅은 ML Kit Image Labeling 또는 동등한 로컬 이미지 라벨링 계층으로 교체합니다.
- 로컬 태그는 빠른 검색용 초안입니다.
- 로컬 태그가 모든 물체, 정확한 개수, 세밀한 위치를 보장한다고 표현하지 않습니다.

### 30.4 로컬 LLM/VLM 고급 AI 분석

- 고급 분석은 로컬 MediaPipe 호환 LLM/VLM 모델이 준비된 경우에만 실행합니다.
- Gemini, BYOK, 외부 AI provider fallback을 새로 추가하지 않습니다.
- 설정에서는 로컬 모델 다운로드/삭제와 필요한 경우 모델 다운로드용 read token 저장/삭제만 제공합니다.
- 분석 전에는 사진이 기기 안에서 처리되며 외부 AI 제공자에게 전송되지 않음을 안내합니다.
- 로컬 모델 응답은 수정 가능한 제안으로만 표시합니다.
...
- 실제 API Key 또는 토큰이 포함되지 않았는지
- `local.properties`, `.env`, `secrets.properties`, keystore가 포함되지 않았는지
- 사진 샘플에 개인정보가 들어 있지 않은지
- 디버그 로그에 사용자 데이터가 남지 않는지
- 앱 권한이 문서와 충돌하지 않는지
