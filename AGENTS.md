...
- 앱은 API Key 없이도 기본 기능이 동작해야 합니다.
- 고급 AI 분석은 BYOK(Bring Your Own Key) 방식의 선택 기능입니다.
...
- 개발자 소유 기본 API Key를 앱에 넣지 않습니다.
- 사용자가 입력한 API Key는 기기 내부에만 암호화 저장합니다.
- API Key, 이미지 바이트, 프롬프트, AI 응답, 민감 데이터는 로그에 남기지 않습니다.
...
- 첫 실행에서 API Key 입력을 강제하지 않습니다.
- 기본 플로우는 `사진 촬영/선택 → 로컬 태그 초안 → 태그 수정 → 로컬 저장 → 검색`입니다.
- API Key가 없어도 사용자는 앱의 핵심 가치를 경험할 수 있어야 합니다.
...
- BYOK는 고급 분석 옵션입니다.
- API Key는 Android Keystore 기반 암호화 저장소 또는 동등한 보안 저장 방식으로 저장합니다.
- 사용자는 설정에서 API Key를 저장, 연결 테스트, 삭제할 수 있어야 합니다.
- 외부 AI 분석 전에는 선택한 사진과 프롬프트가 AI 제공자에게 전송될 수 있음을 안내합니다.
- 개발자 서버를 도입하지 않는 한, 사용자 API Key는 개발자 서버로 전송하지 않습니다.
