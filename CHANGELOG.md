...
### Fixed
- 일부 단말(예: 갤럭시 S24)에서 첫 실행 시 온보딩 화면이 표시된 직후 앱이 즉시 종료되던 현상을 방어 처리했습니다. ML Kit 한국어 OCR 클라이언트 초기화를 첫 사용 시점까지 지연하고, Room Flow 구독과 초기 인증/네비게이션 단계의 비동기 예외가 액티비티를 죽이지 않도록 안전망을 추가했습니다.
