...
### Fixed
- 태블릿 가로모드처럼 화면 높이가 낮을 때, 오늘 탭의 첫 화면(기록이 없을 때)에서 "지금 촬영하기"·"사진 가져오기" 버튼이 화면 아래로 잘려 보이지 않고 스크롤도 되지 않던 문제를 수정했습니다. 이제 내용이 화면보다 길면 스크롤되어 버튼까지 닿을 수 있습니다.

### Build / CI
- Play Store 업로드용 새 산출물을 만들기 위해 앱 버전을 `2.6.9` / `269`로 올렸습니다.

### Verification
- 로컬: `./gradlew :app:assembleDebug` 성공.
- 기기: 무선 연결한 Android 15 태블릿(TB320FC) 가로모드에서 오늘 탭 빈 화면이 스크롤되어 촬영/가져오기 버튼에 도달함을 확인. 수정 전에는 스크롤 컨테이너가 없어 버튼이 잘렸음.

## v2.6.8 - 2026-05-30

### Fixed
- 설정에서 모델 받기·토큰 저장 등을 눌렀을 때 안내 메시지가 화면 밖(설정 페이지 맨 아래)에 표시되어 "아무 반응이 없는" 것처럼 보이던 문제를 수정했습니다. 이제 안내 메시지가 화면 하단에 바로 떠서, 어떤 동작을 했는지(예: "먼저 HuggingFace 토큰을 저장해주세요") 즉시 확인할 수 있습니다.

### Build / CI
- Play Store 업로드용 새 산출물을 만들기 위해 앱 버전을 `2.6.8` / `268`로 올렸습니다.

### Verification
- 로컬: `./gradlew :app:compileDebugKotlin`, `./gradlew :app:assembleDebug` 성공.
- 기기: 무선 연결한 Android 15 태블릿(TB320FC)에서 토큰 없이 모델 받기 버튼을 눌렀을 때, 안내 스낵바가 버튼 바로 아래(화면 안)에 표시됨을 확인. 수정 전에는 동일 메시지가 설정 페이지 최하단에만 그려져 보이지 않았음.

## v2.6.7 - 2026-05-30

### Fixed
- 로컬 고급 AI 모델을 다운로드할 때, 일부 네트워크/저장소 환경에서 다운로드가 시작되지 않거나 실패하던 문제를 수정했습니다. 모델 파일 서버로 자동 연결되는 과정에서 인증 정보가 잘못 전달되어 거부되던 경우를 바로잡아, 라이선스를 수락하고 토큰을 입력한 모델을 더 안정적으로 받을 수 있습니다.

### Build / CI
- Play Store 업로드용 새 산출물을 만들기 위해 앱 버전을 `2.6.7` / `267`로 올렸습니다.

### Verification
- 로컬: `./gradlew :app:testDebugUnitTest`, `./gradlew :app:assembleDebug` 성공.
- 기기: 무선 연결한 Android 15 태블릿(TB320FC)에 디버그 빌드를 설치하고, 실제 HuggingFace 호스트 → CDN cross-host 리다이렉트 경로로 모델 파일 다운로드가 byte 단위까지 정확히(3,561,811 bytes) 완료됨을 확인. 수정 전후 모두 동일 파일이 정상 저장되어 회귀 없음 확인.
