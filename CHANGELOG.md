...
### Fixed
- **빌드 시스템 안정화**: 모든 UI 화면의 리소스 참조 및 임포트 구조를 정비하여 컴파일 오류를 완전히 해결했습니다.
- **의존성 동기화**: ML Kit 및 코루틴 관련 필수 라이브러리(`kotlinx-coroutines-play-services`)를 통합하여 런타임 안정성을 확보했습니다.
...
### Build / CI
- 앱 버전을 `v1.8.3` (versionCode 183)으로 업데이트했습니다.
