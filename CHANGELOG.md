...
### Fixed
- **컴파일 무결성 확보**: `ImageOptimizer`의 시스템 API 참조 오류 및 `MarkSceneApp`의 누락된 데이터베이스 엔터티 임포트를 최종적으로 수정했습니다.
- **리소스 안정성**: 모든 UI 컴포넌트의 레이아웃 API와 리소스 참조를 최신 Android 표준에 맞춰 재정비했습니다.
...
### Build / CI
- 앱 버전을 `v1.8.5` (versionCode 185)으로 업데이트했습니다.
